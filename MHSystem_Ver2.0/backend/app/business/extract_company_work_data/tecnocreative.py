...
def change_firstFormat_tecnocreative(pure_df,file_path):
    ...
    result_df["開始時間"] = result_df["開始時間"].apply(util.convert_to_time)
    result_df["終了時間"] = result_df["終了時間"].apply(util.convert_to_time)
    result_df["休憩時間"] = result_df["休憩時間"].apply(util.convert_to_time)
    result_df["残業時間"] = result_df["残業時間"].apply(util.convert_to_time)

    # 実働時間の計算
    result_df["実働時間"] = result_df.apply(util.calculate_working_time, axis=1)

    # 実働時間を HH:MM 形式にフォーマット
    result_df["実働時間"] = result_df["実働時間"].apply(util.format_timedelta)
    # 開始、終了、休憩、残業を datetime から HH:MM 形式に直す
    result_df["開始時間"] = result_df["開始時間"].apply(lambda x: util.format_time(x) if isinstance(x, datetime) else x)
    result_df["終了時間"] = result_df["終了時間"].apply(lambda x: util.format_time(x) if isinstance(x, datetime) else x)
    result_df["休憩時間"] = result_df["休憩時間"].apply(lambda x: util.format_time(x) if isinstance(x, datetime) else x)
    result_df["残業時間"] = result_df["残業時間"].apply(lambda x: util.format_time(x) if isinstance(x, datetime) else x)
